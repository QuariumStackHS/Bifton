...
#define CurrentFiled Deter->objPath[i]
#define OBJCurrent (SE->Switchs["--Assembly"]) ? get_S_name(CurrentFiled) : get_o_name(CurrentFiled)
        int OK = 0;
        string por;
        for (int i = 0; i < Deter->objPath.size(); i++)
        {
